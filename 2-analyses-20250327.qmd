...
tab_t1_6_pcs <- tibble(metric = c("r2", "citl", "cslope", "c-statistic"),
                       apparent_pcs = c(r2, cal[["citl"]], cal[["cslope"]], NA),
                       internal_pcs = c(cv[["r2"]], cv[["citl"]], cv[["cslope"]], NA))
...
# Calculate pseudo R-squared
r2_t2_6_pcs <- dat_t2_6_pcs %$% pseudo_r2(lps_o, fit_t2_6_pcs[["sigma_o"]])
...
# Develop model
fit_t1_6_mcs <- t1_dev(dat_imputed, "mcs_6_t1", form_rhs)

# Get predictions and make data frame
dat_t1_6_mcs <- t1_prd(dat_imputed, "mcs_6_t1", fit_t1_6_mcs) %>%
    # Bind columns
    bind_cols() %>%
    # Add weights and observed value
    mutate(w = ipcw(dat_imputed, "cens_6"),
           true_y = filter(dat_imputed, .imp == 1)[["mcs_6_t1"]])

# Calculate pseudo R-squared
r2_t1_6_mcs <- dat_t1_6_mcs %$% pseudo_r2(lps, fit_t1_6_mcs[["scale"]])

 # Get model calibration
cal_t1_6_mcs <- t1_cal(dat_t1_6_mcs, "6-month MCS")
...
# Develop model
fit_t2_6_mcs <- t2_dev(dat_imputed, "mcs_6", selection_form_rhs, outcome_form_rhs)

# Get predictions and make data frame
dat_t2_6_mcs <- t2_prd(dat_imputed, "mcs_6", "cens_6", fit_t2_6_mcs) %>%
    # Bind columns
    bind_cols() %>%
    # Add weights and observed value
    mutate(w = 1 / pr,
           true_y = filter(dat_imputed, .imp == 1)[["mcs_6"]])

# Calculate pseudo R-squared
r2_t2_6_mcs <- dat_t2_6_mcs %$% pseudo_r2(lps_o, fit_t2_6_mcs[["sigma_o"]])

# Get model calibration
cal_t2_6_mcs <- t2_cal(dat_t2_6_mcs, "6-month MCS")
